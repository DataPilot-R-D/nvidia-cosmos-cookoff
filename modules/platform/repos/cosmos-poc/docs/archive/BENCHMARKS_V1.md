...
For PAIC2 surveillance, we need both fast perception AND intelligent reasoning. This comparison helps define which model handles what in the pipeline.
...
### Recommended architecture for PAIC2
...
The roses test revealed that **targeted, specific prompts** ("was anything moved?") succeed where general prompts fail completely. For PAIC2 integration, invest heavily in prompt templates.
...
- **Critical finding for PAIC2: Person detection is unreliable at 2B scale.** Cannot be primary person detector — need dedicated model (YOLO/person detection) as pre-filter
