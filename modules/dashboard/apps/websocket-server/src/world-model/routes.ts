...
export function handleWorldModelRoutes(
  ...
  // ── Constraints ──────────────────────────────────────
  if (pathname === '/api/constraints' && method === 'GET') {
    const url = new URL(req.url)
    const zoneId = url.searchParams.get('zoneId') ?? undefined
    return json(listConstraints(db, zoneId))
  }
  if (pathname === '/api/constraints' && method === 'POST') {
    return handleCreateConstraint(req, db)
  }

  const constraintMatch = pathname.match(/^\/api\/constraints\/([a-f0-9-]+)$/)
  if (constraintMatch) {
    const id = constraintMatch[1]!
    if (method === 'GET') {
      const c = getConstraint(db, id)
      return c ? json(c) : errorJson('Constraint not found', 404)
    }
    if (method === 'PATCH') return handleUpdateConstraint(req, id, db)
    if (method === 'DELETE') {
      return deleteConstraint(db, id) ? json({ ok: true }) : errorJson('Constraint not found', 404)
    }
  }
...
async function handleCreateZone(req: Request, db: Database): Promise<Response> {
  try {
    ...
    if (msg.includes('Polygon must be closed')) return errorJson(msg, 400)
    if (msg.includes('UNIQUE constraint')) return errorJson('Zone name must be unique', 409)
...
async function handleCreateConstraint(req: Request, db: Database): Promise<Response> {
  try {
    const body = await req.json()
    const parsed = CreateConstraintSchema.safeParse(body)
    if (!parsed.success) return errorJson(parsed.error.issues.map((i) => i.message).join('; '), 400)
    return json(createConstraint(db, parsed.data), 201)
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Invalid JSON body'
    if (msg.includes('FOREIGN KEY')) return errorJson('Referenced zone does not exist', 400)
    return errorJson(msg, 400)
  }
}

async function handleUpdateConstraint(req: Request, id: string, db: Database): Promise<Response> {
  try {
    const body = await req.json()
    const parsed = UpdateConstraintSchema.safeParse(body)
    if (!parsed.success) return errorJson(parsed.error.issues.map((i) => i.message).join('; '), 400)
    const constraint = updateConstraint(db, id, parsed.data)
    return constraint ? json(constraint) : errorJson('Constraint not found', 404)
  } catch {
    return errorJson('Invalid JSON body', 400)
  }
}
