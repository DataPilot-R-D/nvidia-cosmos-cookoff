...
const cameraSourceRegistry = await createCameraSourceRegistry({ logger })
...
const server = Bun.serve<WebSocketData>({
  ...
  async fetch(req, server) {
    ...
    // Camera API
    if (pathname.startsWith('/api/cameras/sources')) {
      const cameraSourceResponse = handleCameraSourceRoutes(req, pathname, cameraSourceRegistry)
      if (cameraSourceResponse) {
        const resolved =
          cameraSourceResponse instanceof Promise
            ? await cameraSourceResponse
...
const shutdown = () => {
  ...
  void cameraSourceRegistry.shutdown()
  server.stop()
  io.close(() => {
