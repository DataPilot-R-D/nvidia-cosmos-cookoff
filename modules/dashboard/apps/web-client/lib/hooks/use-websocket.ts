...
export function useWebSocket(initialUrl: string): UseWebSocketReturn {
  ...
  /**
   * Handle dashboard_notification from ROS2 planner/executor via rosbridge
   */
  const handleDashboardNotification = useCallback((data: unknown) => {
    if (!mountedRef.current) return
    ...
    if (typeof data === 'object' && data !== null) {
      ...
      const payload = message.data
      if (payload?.title) {
        const validLevels: NotificationLevel[] = ['info', 'warning', 'error', 'success']
        const level: NotificationLevel = validLevels.includes(payload.level as NotificationLevel)
          ? (payload.level as NotificationLevel)
          : 'info'

        useNotificationStore.getState().add(level, payload.title, payload.message, payload.category)
      }
    }
  }, [])
