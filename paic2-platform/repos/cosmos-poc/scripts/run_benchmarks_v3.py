#!/usr/bin/env python3
"""Full benchmark retest — 32768 context, reasoning via format instruction in prompt."""
...
def run_streaming_ttft(content_items, prompt):
    ...
    resp = requests.post(BASE, json={
        ...
        "temperature": 0.6, "top_p": 0.95, "max_tokens": BENCHMARK_MAX_TOKENS,
        "stream": True
...
def log(test_id, desc, result):
    ...
    print(f"{test_id:10s} | {desc[:40]:40s} | {lat:>6}s | {ptok}/{ctok} | think:{think} | {fr} | {answer}")
    sys.stdout.flush()

print("=" * 140)
print(f"COSMOS REASON2-8B FULL RETEST V3 — ctx={_MAX_MODEL_LEN}, max_tokens={BENCHMARK_MAX_TOKENS}, json_cap={BENCHMARK_JSON_MAX_TOKENS}")
print("=" * 140)
...
out_path = Path(__file__).parent.parent / "tests" / "benchmark_v3_raw.json"
