...
export interface OccupancyGridData {
  ...
  data: string // base64 encoded Int8Array
}

/**
 * Result of map analysis for exploration
 */
...
/** Minimum distance to consider a frontier (meters) */
const MIN_FRONTIER_DISTANCE = 0.5

/** Maximum distance to consider a frontier (meters) */
const MAX_FRONTIER_DISTANCE = 15.0
...
/**
 * Decode base64 OccupancyGrid data to Int8Array
 */
export function decodeOccupancyGrid(base64Data: string): Int8Array {
...
export function selectBestFrontier(
  ...
  for (const frontier of frontiers) {
    ...
    if (dist < MIN_FRONTIER_DISTANCE) continue

    // Skip frontiers that are too far (exploration efficiency)
    if (dist > MAX_FRONTIER_DISTANCE) continue
...
/**
 * Parse OccupancyGrid message from ROS and analyze for exploration
 *
 * @param occupancyGrid - Raw OccupancyGrid data from ROS
 * @param robotX - Robot's current X position
 * @param robotY - Robot's current Y position
 * @returns Exploration analysis or null if invalid data
 */
export function analyzeOccupancyGrid(
  occupancyGrid: OccupancyGridData,
  robotX: number,
  robotY: number
): ExplorationAnalysis | null {
  try {
    // Decode base64 data
    const gridData = decodeOccupancyGrid(occupancyGrid.data)
    ...
    const metadata: MapMetadata = {
      ...
      originX: occupancyGrid.origin.x,
      originY: occupancyGrid.origin.y,
    }

    // Analyze map
    return analyzeMapForExploration(gridData, metadata, robotX, robotY)
  } catch (error) {
    console.error('Failed to analyze occupancy grid:', error)
    return null
  }
}
