...
export function CameraTile({
  ...
  const canReportIncident = usePermission('incident:manage')

  const handleClick = useCallback(() => {
    if (!showIncidentForm) onClick?.(source.id)
  }, [onClick, source.id, showIncidentForm])

  const handleReportIncident = useCallback((e: React.MouseEvent) => {
    e.stopPropagation()
  ...
  return (
    <div
      className={`
        group relative flex flex-col bg-[#0d0f11] border rounded overflow-hidden cursor-pointer
    ...
    >
      ...
      {/* Report Incident button (top-right) */}
      {canReportIncident && enabled && !showIncidentForm && (
        <button
          type="button"
          onClick={handleReportIncident}
          className="absolute top-1 right-1 px-1.5 py-0.5 text-[7px] font-mono uppercase rounded bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30 opacity-0 group-hover:opacity-100 transition-opacity"
          title="Report Incident"
          data-testid={`report-incident-${source.id}`}
        >
          ⚠ Report
        </button>
      )}

      {/* Incident Report Form overlay */}
      {showIncidentForm && (
        <IncidentReportForm
