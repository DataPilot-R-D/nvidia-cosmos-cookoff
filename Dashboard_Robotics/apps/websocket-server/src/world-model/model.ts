...
export const ZoneTypeSchema = z.enum(['patrol', 'restricted', 'charging', 'staging'])
...
/**
 * Validate polygon is closed (first point === last point) if it has >= 3 points.
 */
function validatePolygonClosed(polygon: number[][]): void {
  if (polygon.length >= 3) {
...
export function createZone(db: Database, input: CreateZoneInput): Zone {
  ...
  if (parsed.polygon.length > 0) validatePolygonClosed(parsed.polygon)
...
export function updateZone(db: Database, id: string, input: UpdateZoneInput): Zone | null {
  ...
  if (parsed.polygon && parsed.polygon.length > 0) validatePolygonClosed(parsed.polygon)
  ...
  const ALLOWED_COLUMNS = ['name', 'type', 'color', 'maxRobots'] as const

  if (parsed.polygon !== undefined) {
    sets.push('polygon = $polygon')
    params.$polygon = JSON.stringify(parsed.polygon)
  }
...
export const ConstraintTypeSchema = z.enum(['speed-limit', 'no-entry', 'time-window'])
...
export function createConstraint(db: Database, input: CreateConstraintInput): Constraint {
  const parsed = CreateConstraintSchema.parse(input)
  const now = new Date().toISOString()
  const id = randomUUID()

  db.prepare(
    `INSERT INTO constraints (id, type, zoneId, params, description, createdAt, updatedAt)
     VALUES ($id, $type, $zoneId, $params, $description, $createdAt, $updatedAt)`
  ).run({
    $id: id,
    $type: parsed.type,
...
export function getConstraint(db: Database, id: string): Constraint | null {
  const row = db.prepare('SELECT * FROM constraints WHERE id = ?').get(id) as ConstraintRow | null
  return row ? rowToConstraint(row) : null
}

export function listConstraints(db: Database, zoneId?: string): Constraint[] {
  if (zoneId) {
    const rows = db
      .prepare('SELECT * FROM constraints WHERE zoneId = ? ORDER BY createdAt DESC')
      .all(zoneId) as ConstraintRow[]
    return rows.map(rowToConstraint)
  }
  const rows = db
    .prepare('SELECT * FROM constraints ORDER BY createdAt DESC')
...
export function updateConstraint(
  ...
  const parsed = UpdateConstraintSchema.parse(input)
  const existing = getConstraint(db, id)
  if (!existing) return null
  ...
  if (parsed.zoneId !== undefined) {
    sets.push('zoneId = $zoneId')
    params.$zoneId = parsed.zoneId
  }
  if (parsed.params !== undefined) {
    sets.push('params = $params')
    params.$params = JSON.stringify(parsed.params)
  }
  if (parsed.description !== undefined) {
    sets.push('description = $description')
    params.$description = parsed.description ?? null
  }
  ...
  db.prepare(`UPDATE constraints SET ${sets.join(', ')} WHERE id = $id`).run(params)
  return getConstraint(db, id)
}

export function deleteConstraint(db: Database, id: string): boolean {
  const result = db.prepare('DELETE FROM constraints WHERE id = ?').run(id)
  return result.changes > 0
}
